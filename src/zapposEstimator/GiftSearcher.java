/* Searches for the appropriate gifts in the Database (Zappos API) */
...
public class GiftSearcher {
	...
	private void setProductCombosRecursive(ArrayList<Product> productList, double target, ArrayList<Product> partial) {
		...
		if(Math.abs(sum - target) < priceWithinAmount && partial.size() == numItems && productCombos.size() < MAXCOMBOS) {
			...
			if(productCombos.isEmpty()) {	productCombos.add(new ProductCombo(partial, totalPrice)); }
	...
	public String getGiftCombos(){
		...
		String step1 = "Searching Zappos...";
                String line = "\n";
		this.setProductsInRange();
		
		String step2 = "Finding combinations that work for you...";
		//convert to Products
		...
		if(!productCombos.isEmpty()) {
			...
			return step1+line+step2+line+toPrint;
		}
		else {
            ...
            JOptionPane.showMessageDialog(null, "Something went wrong.Check for unauthorised access or server load"
